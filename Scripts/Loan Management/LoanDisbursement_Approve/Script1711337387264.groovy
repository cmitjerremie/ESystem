...
def disburseLoan = findTestData('LoanDisbursement')

// Get the number of rows in the data file
int rowCount = disburseLoan.getRowNumbers()

if (rowCount >= 1) {
	// Iterate through rows
	for (int i = rowCount; i >= 1; i--) {
		String cid = disburseLoan.getValue('CID', i)
		...
		if (!tableContent.equals('No matching records found')) {
			...
			String msg_rls = WebUI.getText(findTestObject('Object Repository/LoanManagementModule/LoanDisbursement/div_Loan has been Released'))
	...
	println("Error: Data file contains no rows. - Please add and approve loan first!")
	KeywordUtil.markFailed("Error: Data file contains no rows. - Please add and approve loan first!");
}